...
export default function ModalPesquisa({ isOpen, onClose }) {
  ...
  const handleTempoChange = (e) => setTempoSelecionado(e.target.value);

  const handleBuscarClick = () => {
    const queryParams = new URLSearchParams();

    if (selectedRating > 0) queryParams.append('avaliacaoMin', selectedRating);
    ...
    if (tempoSelecionado) {
      ...
      if (tempoMinimo) queryParams.append('tempoMin', tempoMinimo);
      if (tempoMaximo) queryParams.append('tempoMax', tempoMaximo);
    }

    if (recipeType && recipeType !== 'Selecione') queryParams.append('titulo', recipeType);

    const url = `/joao_vitor/resulBuscaAvancada?${queryParams.toString()}`;
    console.log('URL de busca avançada:', url);
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', position: 'fixed', top: 0, left: 0, backgroundColor: 'rgba(0, 0, 0, 0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
      <div style={{ width: '75vw', height: '75vh', backgroundColor: 'white', padding: '20px', borderRadius: '10px', position: 'relative', overflow: 'hidden' }}>
        <div style={{ height: '100%', overflowY: 'auto', paddingRight: '10px' }}>
          ...
          <div style={{ marginTop: '30px', textAlign: 'center' }}>
            <button
              ...
              style={{ padding: '10px 20px', backgroundColor: '#FF914D', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>
              Buscar
            </button>
          </div>
        </div>
